...
(
	...
	reg [6:0] fw_1000000, fw_100000, fw_10000, fw_1000, fw_100, fw_10, fw_2;
	reg [63:0] mult_1000000, mult_100000, mult_10000, mult_1000, mult_100, mult_10, mult_2;
	always @(posedge clk) begin
	
		if(rst_n == 0) begin
			//reset condition
			{fw_1000000, fw_100000, fw_10000, fw_1000, fw_100, fw_10, fw_2} <= 0;
			{mult_1000000, mult_100000, mult_10000, mult_1000, mult_100, mult_10, mult_2} <= 0;
		end
		else begin
		
			/*
			fw_1000000 <= (1024*final_freq)/(CLK_FREQ/1000000) - 1;
			fw_100000 <= (1024*final_freq)/(CLK_FREQ/100000) - 1;
			fw_10000 <= (1024*final_freq)/(CLK_FREQ/10000) - 1;
			fw_1000 <= (1024*final_freq)/(CLK_FREQ/1000) - 1;
			fw_100 <= (1024*final_freq)/(CLK_FREQ/100) - 1;
			fw_10 <= (1024*final_freq)/(CLK_FREQ/10) - 1;
			fw_2 <= (1024*final_freq)/(CLK_FREQ/2) - 1;
			*/
			
			mult_1000000 <= (final_freq<<19)+(final_freq<<18)+(final_freq<<17)+(final_freq<<16)+(final_freq<<14)+(final_freq<<9)+(final_freq<<6);
			mult_100000 <= (final_freq<<16)+(final_freq<<15)+(final_freq<<10)+(final_freq<<9)+(final_freq<<7)+(final_freq<<5);
			mult_10000 <= (final_freq<<13)+(final_freq<<10)+(final_freq<<9)+(final_freq<<8)+(final_freq<<4);
			mult_1000 <= (final_freq<<9)+(final_freq<<8)+(final_freq<<7)+(final_freq<<6)+(final_freq<<5)+(final_freq<<3);
			mult_100 <= (final_freq<<6)+(final_freq<<5)+(final_freq<<2);
			mult_10 <= (final_freq<<3)+(final_freq<<1);
			mult_2 <= (final_freq<<1);
